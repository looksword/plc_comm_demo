...
namespace Gnm.Utility
{
	/// <summary>
	/// Gnm utility methods.
	/// </summary>
	public static class GnmUtility
	{
        ...
        public static byte[] AnalysisAddress(byte functioncode, ushort startaddress)
        {
            byte[] ActualAddr = null;
            switch(functioncode)
            {
                case Gnm.ReadCoils:
                    {
                        ActualAddr = GnmUtility.GetAsciiBytes((ushort)((startaddress / 16) * 2 + 0x100));
                        break;
                    }
                case Gnm.ReadXarea:
                    {
                        ActualAddr = GnmUtility.GetAsciiBytes((ushort)((startaddress / 16) * 2 + 0x80));
                        break;
                    }
                case Gnm.ReadYarea:
                    {
                        ActualAddr = GnmUtility.GetAsciiBytes((ushort)((startaddress / 16) * 2 + 0xA0));
                        break;
                    }
            }
            return ActualAddr;
