...
namespace HslCommunication.Core
{
    ...
    public static class ByteTransformHelper
    {
        ...
        public static OperateResult<TResult> GetResultFromBytes<TResult>(OperateResult<byte[]> result, Func<byte[], TResult> translator)
        {
            ...
            {
                return new OperateResult<TResult>() { Message = StringResources.Language.DataTransformError + SoftBasic.ByteToHexString(result.Content) + string.Format(" : Length({0}) ", result.Content.Length) + ex.Message };
            }
        }
