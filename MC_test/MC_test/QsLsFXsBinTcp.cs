...
namespace MC_test
{
    public class QsLsFXsBinTcp : IMelsecMaster
    {
        // Fields
        ...
        {
            string ins = "50-00-00-FF-FF-03-00-0C-00-" + this.GetMonitorTime() + "01-04-00-00-" + this.GetStartDevice(devType, firstDevIndex) +
        ...
        {
            string ins = "50-00-00-FF-FF-03-00-" + this.GetDataLength(iStep) + this.GetMonitorTime() + "01-14-00-00-" +
        ...
        public int Open()
        {
            try
            {
                ...
                if (this.socket.Receive(buffer2) != 13)
                {
                    throw new Exception();
                }
                return 0;
